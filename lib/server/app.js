...
function create (env, ctx) {
  ...
  if (env.settings.isEnabled('cors')) {
    ...
    app.use(function allowCrossDomain (req, res, next) {
      ...
      // Always allow same-origin requests (Nightscout website itself)
      if (!origin || (origin && origin.includes(host))) {
        allowedOrigin = origin || allowOrigin;
      }
      // Allow requests from dddiaz.com
      else if (origin === 'https://dddiaz.com' || 
               origin === 'http://dddiaz.com' ||
               origin.endsWith('.dddiaz.com')) {
        allowedOrigin = origin;
      }
      // If allowOrigin is explicitly set and not '*', respect that setting
      else if (allowOrigin && allowOrigin !== '*') {
        allowedOrigin = allowOrigin;
      }
      
      // For API endpoints, be more restrictive
      if (req.path.startsWith('/api/') && !allowedOrigin && origin) {
        console.warn('CORS blocked for API endpoint:', { 
          path: req.path, 
          origin: origin, 
          host: host,
        ...
        return res.status(403).json({
          ...
          message: 'API access not allowed from this origin'
        });
      }
      
      res.header('Access-Control-Allow-Origin', allowedOrigin || allowOrigin);
